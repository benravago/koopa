...
public final class Tokens {
     ...
     * Will also retain the ranges from the original token,
     * except for those getting cut by the substring semantics.
     *
     * @param token The input token.
     * @param beginIndex Relative offset to start from.
     * @return The requested subtoken.
     */
    public static Token subtoken(Token token, int beginIndex) {
        var lengthOfToken = token.getLength();

        // Check bounds.
        if (beginIndex < 0 || beginIndex > lengthOfToken) {
            throw new IndexOutOfBoundsException(
                "Start position (" + beginIndex + ") falls outside the token's bounds [0, " + lengthOfToken + "[." );
        }
        ...
        if (beginIndex == lengthOfToken) {
            var ranges = new ArrayList<Range>(1);
        ...
        var text = token.getText().substring(index);

        var ranges = new ArrayList<Range>();
        for (var range : token.getRanges()) {
            if (index == 0) {
                // We have already made the cut. All further ranges get added as-is.
                ranges.add(range);
                continue;
            }
            ...
            if (index < length) {
                ...
                var start = range.getStart().offsetBy(index);
            ...
            // And this case is for when we haven't yet reached the correct range to cut.
            index -= length;
        }
     ...
     * <p>
     * Will return <code>token</code> if the begin index is zero and the end index is equal to this token's length.
     * <p>
     * Will copy all tags to the new token.
     *
     * @param token The input token.
     * @param beginIndex Relative offset to start from.
     * @param endIndex Relative offset to end at (not inclusive).
     * @return The requested subtoken.
     */
     ...
     * <p>
     * In case the cut happens at the start or at the end then
     * one of the tokens will be <code>null</code>
     * and the other will be <code>token</code>.
     * <p>
     * Will copy all tags to the new token.
     *
     * @param token The input token.
     * @param cutIndex Relative offset to cut at.
     *     The first token will go up to this position, but not include it.
     *     The second token will start at this position.
     *
    ...
    public static Token[] split(Token token, int cutIndex) {
        ...
        return new Token[]{ subtoken(token, 0, cutIndex), subtoken(token, cutIndex) };
    }
