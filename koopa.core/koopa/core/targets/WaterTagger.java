 ...
 * and replaces them with {@linkplain IslandTag#LAND}
 * and {@linkplain IslandTag#WATER} on the tokens themselves.
 */
...
public class WaterTagger implements Target {
    ...
    @Override
    public void done() {
        // We want to make sure that the water tagger is not responsible
        // for hanging on to memory which could otherwise be freed up.
        // For instance, the target may include tree builders or token trackers,
        // which can be expensive and are something we want to make sure can be reclaimed.
        if (target != null) {
            target.done();
            target = null;
        }
    ...
    public void push(Data data) {
        ...
        if (data instanceof Start && "unknown".equals(((Start) data).getName())) {
            inUnknown = true;
        }
        ...
        if (data instanceof Token) {
            data = ((Token)data).withTags((inWater || inUnknown) ? WATER : LAND);
        }
        if (target != null) {
            target.push(data);
        }
