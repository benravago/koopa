...
/**
 * This {@linkplain ParserCombinator} tracks the nesting (opening and closing) of parentheses
 * (or whatever you tell it to balance) during the matching of another given {@linkplain ParserCombinator}.
 * 
 * At any time during that match you can ask this {@linkplain ParserCombinator} whether or not it is in a balanced state
 * (which is defined at {@linkplain Balancer#isBalanced()}).
 */
public class Balancing extends UnaryParserDecorator {

    /**
     * Continuous testing of the {@link #pairs} introduces a lot of extra logging.
     * This flag toggles it off by default.
     */
     ...
     * <p>
     * They will be placed in a {@linkplain Closure} at the time the actual {@linkplain Balancer} starts matching.
     */
    private final Pair[] pairs;

    public Balancing(Grammar grammar, ParserCombinator parser, ParserCombinator... pairs) {
        super(parser);
    ...
    protected boolean matches(Parse parse) {
        ...
        var newBalancer = new Balancer(parser, parse);
        addObserver(target, newBalancer);
        var accepts = newBalancer.accepts(parse);
    ...
    /**
     * A pair of {@linkplain ParserCombinator}s marking the start and end of something.
     */
    private class Pair {
    ...
    /**
     * This observes the {@linkplain Data} being pushed/popped by the {@linkplain Stream}.
     */
    public class Balancer extends UnaryParserDecorator implements HoldingTarget.Observer {
        ...
        private final Deque<Match> pushed = new LinkedList<>();
        private final Deque<Match> popped = new LinkedList<>();
        private final Deque<Pair> open = new LinkedList<>();
        ...
        public Balancer(ParserCombinator parser, Parse parse) {
            ...
            for (var i = 0; i < pairs.length; i++) {
                var pair = pairs[i];
                this.closedPairs[i] = new Pair(new Closure(pair.open, parse), new Closure(pair.close, parse));
            }
        }
        ...
        protected boolean matches(Parse parse) {
            ...
            if (parse.getTrace().isEnabled()) {
                parse.getTrace().dedent(toString() + " : " + (accepts ? "yes" : "no"));
            }
            return accepts;
        }

        /**
         * This is the whole reason for this class: so that we may ask whether or not we're inside a balanced set of parentheses.
         * <p>
        ...
        public void pushed(HoldingTarget holdingTarget, Token token) {
            ...
            if (parse.getTrace().isEnabled()) {
                parse.getTrace().add("# " + hashCode() + ", pushed(" + token + ") ...");
            }
            if (!popped.isEmpty()) {
        ...
        public void popping(HoldingTarget holdingTarget, Token last) {
            ...
            if (parse.getTrace().isEnabled()) {
                parse.getTrace().add("# " + hashCode() + ", popping(" + last + ") ...");
            }
            pop(last);
            if (SILENCE) {
                parse.getTrace().silence(false);
        ...
        private boolean tryClosing(Stream stream) {
            ...
            stream.bookmark();
            var couldClose = pair.close.accepts(parse);
            stream.rewind();
            if (parse.getTrace().isEnabled()) {
                parse.getTrace().dedent(toString() + ", close " + pair.close + " : " + (couldClose ? "yes" : "no"));
            }
            return couldClose;
        }
        ...
        private boolean tryOpening(Stream stream, Pair pair) {
            if (parse.getTrace().isEnabled()) {
                parse.getTrace().indent(toString() + ", open " + pair.open + " ?");
            }
            stream.bookmark();
            var couldOpen = pair.open.accepts(parse);
            stream.rewind();
            if (parse.getTrace().isEnabled()) {
                parse.getTrace().dedent(toString() + ", open " + pair.open + " : " + (couldOpen ? "yes" : "no"));
            }
            return couldOpen;
        }
        ...
        private void pop(Data data) {
            while (!pushed.isEmpty() && pushed.peek().data == data) {
                var match = pushed.removeFirst();
                // If this looks strange, don't forget we're inverting the logic here.
                if (match.kind == Kind.OPEN) {
                    close(match.pair);
                } else {
                    open(match.pair);
                ...
                if (parse.getTrace().isEnabled()) {
                    parse.getTrace().add("# " + hashCode() + ", popped " + match);
                    parse.getTrace().add(getReport());
                }
            }
        }
        ...
        private void recyclePopped(Data data) {
            while (!popped.isEmpty() && popped.peek().data == data) {
                ...
                if (parse.getTrace().isEnabled()) {
                    parse.getTrace().add("# " + hashCode() + ", recycled " + match);
                }
            }
            if (parse.getTrace().isEnabled()) {
                parse.getTrace().add(getReport());
        ...
        private boolean wasJustClosed() {
            ...
            var lastMatch = pushed.peek();
            if (lastMatch.kind != Kind.CLOSE) {
                return false;
            }
            // TODO Do we care about seeing skipped tokens ?
            var lastData = parse.getFlow().getBaseStream().getTarget().peekAtLastToken();
