...
/**
 * This {@linkplain ParserCombinator} sets up a context during parsing for counting
 * the number of matches a given {@linkplain ParserCombinator} has within that context.
 */
public class Counting extends UnaryParserDecorator {
    ...
    protected boolean matches(Parse parse) {
        var counter = new Counter(parser);
        var accepts = counter.accepts(parse);
        counter.clear();
        return accepts;
    }
    ...
    public class Counter extends UnaryParserDecorator {
        ...
        private final Map<ParserCombinator, Integer> counts = new HashMap<>();
        ...
        /**
         * Get the number of matches for the given parser.
         * Returns zero if we haven't seen it yet.
         */
