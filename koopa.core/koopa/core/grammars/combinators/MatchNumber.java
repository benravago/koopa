...
/**
 * Accepts a single token if it is tagged with {@linkplain SyntacticTag#NUMBER} and its text matches the given text.
 */
public class MatchNumber extends GrammaticalCombinator {
    ...
    protected boolean matchesAfterSkipped(Parse parse) {
        ...
        if (!t.hasTag(SyntacticTag.NUMBER)) {
            if (parse.getTrace().isEnabled()) {
                parse.getTrace().add(toString() + " : no, not tagged as a NUMBER, " + t);
            }
            return false;
